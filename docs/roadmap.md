...
| Agent behavior / 智能体行为     | v2-first Agent EXCON Skill and role-aware Yongding references / v2 默认 Skill 与永定河角色参考                                                                                                                                                                   | Full evaluator-driven rework loop remains incomplete / evaluator 驱动的完整重做闭环未完成                                                                   |
| MCP                             | 17 v2 stdio participant tools mapped to implemented HTTP routes, including Receipt-gated Submission recovery; explicit v1 mode / 17 个与实际 HTTP 路由一致的 v2 stdio 工具，包括 Receipt-gated Submission 安全恢复；显式 v1 模式                                 | Recovery returns only exact immutable snapshots already receipted to that RunAgent / 恢复仅返回已向该 RunAgent 发放 Receipt 的精确不可变快照                |
...
- **M3 — Agent protocol / 智能体协议: partial.** The v2 Skill, 17 MCP tools, and Receipt-gated participant-safe Submission recovery are delivered. Full four-role evaluator → rework → resubmit acceptance remains.
...
2. Complete evaluator → `EVALUATING` → `REWORK`/`ACCEPTED` → granted revision/resubmission, while keeping deterministic verdicts outside the LLM. / 打通完整评价与重做链路。
3. Move v1 onto an explicit facade backed by v2 facts, reconcile unfinished Episodes, then backfill cold history. / 将 v1 迁移为 v2 事实上的显式 facade。
