...
Publish one immutable Artifact with `artifactKey=water-evidence-register` to yourself and `dispatch-coordination`. Its content must satisfy the Task output schema: `evidenceRegister`, `inflowSummary`, and `evidenceRefs`. Send the coordinator a short Message naming the exact ArtifactVersion. Submit the Task with the case-input Receipt and your own receipted ArtifactVersion, using exactly `submissionType=water-evidence-register`, `targetScope=role`, and `endorsementRecipientRunAgentIds=[]`.
