...
describe('Data PostgreSQL CI', () => {
  it('exposes focused deep-test commands without changing the default suite', () => {
    ...
    expect(scripts['test:postgres:data-api']).toBe(
      'pnpm --filter @wiser/api exec vitest run --config vitest.config.ts test/data-postgres-command-executors.spec.ts test/data-postgres-geo-query.spec.ts test/data-reconciliation.integration.spec.ts',
    );
