...
describe('Data PostgreSQL CI', () => {
  it('exposes focused deep-test commands without changing the default suite', () => {
    ...
    expect(scripts['test:postgres:data-api']).toBe(
      'pnpm --filter @wiser/api exec vitest run --no-file-parallelism --config vitest.config.ts test/data-postgres-command-executors.spec.ts test/data-postgres-geo-query.spec.ts test/data-reconciliation.integration.spec.ts test/data-assessment.integration.spec.ts test/data-knowledge-relations.integration.spec.ts test/data-management-catalog.integration.spec.ts',
    );
