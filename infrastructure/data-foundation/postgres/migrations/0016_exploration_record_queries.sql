-- Shared, deterministic predicates for record pages, aggregates and query tiles.
...
language sql immutable parallel safe strict set search_path=pg_catalog as $$
  select case when jsonb_typeof(value) in ('number','string')
    ...
    and btrim(value #>> '{}') ~ '^[+-]?([0-9]+([.][0-9]*)?|[.][0-9]+)([eE][+-]?[0-9]{1,3})?$'
    and pg_input_is_valid(btrim(value #>> '{}'),'numeric')
    then btrim(value #>> '{}')::numeric else null end
$$;
...
language sql immutable parallel safe set search_path=pg_catalog as $$
  select not exists (
    select 1 from jsonb_array_elements(coalesce(filters,'[]'::jsonb)) f
    where not coalesce(case f->>'type'
...
-- Apply the same pinned record conditions before tile selection and clustering.
...
declare
  ...
  requested_purpose text; ceiling text; policy bigint; refs jsonb; record_query jsonb;
...
begin
  ...
  select s.version_refs,s.spec->'recordQuery' into refs,record_query from service.exploration_snapshot s
