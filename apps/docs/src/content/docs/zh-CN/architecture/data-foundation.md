...
lastReviewedCommit: 8ffdb9625985f7cd027658bee222cd6538aafb53
---
...
`infrastructure/data-foundation/parser` 来源解析组件固定使用 openpyxl 3.1.5 的只读工作簿、xlrd 2.0.2 的旧版 XLS 读取与 pypdf 6.18.0。它保留工作表/原始行位置、公式文本和日期语义，将文档内容作为普通文本提取，并校验压缩包路径、加密、展开预算与成员哈希。只有成员清单时明确标记部分可用，不能替代成员内容解析。Python 依赖使用版本和哈希锁定。
