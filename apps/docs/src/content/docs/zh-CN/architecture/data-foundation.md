...
当前默认 API 与 Worker 接入 `DeterministicFakeEmbedding`（`1.0.0-fixture`），用于可复现的集成行为，并非生产语义相关性模型。搜索证据可能包含来源登记清单；Web 说明其检查范围，并将原始摘录与结果摘要分开，不改写权威事实。生产智能分析仍需要按内容类型索引、生产嵌入适配器、相关性评测和受治理的计划执行。Neo4j 继续承担关联发现和投影，不替代 HTTP 授权或 PostgreSQL/PostGIS 权威数据。现有 Web 入口把精确版本连接到资源、记录、地图、图谱与统计视图，不宣称已经能够生成分析答案。
