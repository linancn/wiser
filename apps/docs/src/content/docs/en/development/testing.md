...
`scripts/data-foundation/prepare-ci.mjs` reads and validates the merged Compose configuration without logging credentials, then overlaps two independent branches: `supabase:start → supabase:reset`, and Chromium installation followed by native builds of every buildable service. Missing remote images are pulled during runtime startup to limit competing Docker downloads. Empty build or remote-service selections are rejected before startup, so an empty argument list cannot expand into an unintended whole-profile command.
