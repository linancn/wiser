...
`scripts/data-foundation/prepare-ci.mjs` reads and validates the merged Compose configuration without logging credentials, then overlaps three independent branches: `supabase:start → supabase:reset`, native builds of every buildable service, and pulls of missing remote images. Local image aliases are excluded from pulls while their build is in progress. Empty build or remote-service selections are rejected before startup, so an empty argument list cannot expand into an unintended whole-profile command.

The initializer is restricted to disposable GitHub-hosted runners, waits for all three branches to settle, and propagates any failure before runtime startup. CI then uses `pnpm data:up --no-build`; all migration, seed, service-health, parser, smoke, authenticated-browser, and PostgreSQL checks still run. Each job uses fresh databases and native Docker storage. Runtime credentials, database volumes, and smoke state are never cached.
