...
lastReviewedCommit: 6f95047118c1381282a7ec915038877fba396c04
---
...
`pnpm data:up --no-build` is available when both local application and parser images have already been built from the intended revision. It still waits for service health and preserves all runtime Auth and migration dependencies. The default command continues to build; the CI-only `prepare-ci.mjs` initializer refuses local or self-hosted environments because it resets Supabase. See [Testing and verification](/en/development/testing/) for the CI sequence.
