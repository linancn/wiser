...
| Journey            | What the user sees first                                              | Next useful action                                                                           |
| ------------------ | --------------------------------------------------------------------- | -------------------------------------------------------------------------------------------- |
| Portal and sign-in | Two business systems and one organization account                     | Sign in, then enter an authorized project                                                    |
| Data Foundation    | A search and exploration workspace with current project scope         | Choose a source, read its fixed version, then inspect records, map, relations, or statistics |
| Data intake        | The task ID and its current checks, review, and publication state     | Open the task from the ID supplied by the agent; address the named issue or wait for review  |
| Access management  | Current membership and resource access, separate from request history | Request access or, if authorized, review and execute a grant                                 |
| Agent EXCON        | Scenario and run state, followed by collaboration and evaluation      | Open one run to inspect handoffs, evaluation, execution trace, or replay                     |
