...
| Web                 | Chinese-default multi-scenario read-only reference/live modes, per-agent traces, authority-aware evaluation/Barrier/revision diagnostics, signal coverage, and perspective replay | Live reports missing data explicitly and never falls back to fixtures or fabricates activity         |
