...
lastReviewedCommit: 3f5c3e0d241397491b643e879b8c35c451d7ffef
---
...
The current default API and worker wire `DeterministicFakeEmbedding` (`1.0.0-fixture`) for reproducible integration behavior. This is not a production semantic-relevance model. Search evidence can contain source-registration manifests; the Web labels their check scope and separates original excerpts from the result summary, without changing authority facts. Production intelligent analysis still requires content-specific indexing, a production embedding adapter, relevance evaluation and governed plan execution. Neo4j continues to provide relationship discovery and projections; it does not replace HTTP authorization or PostgreSQL/PostGIS authority. The present Web exploration entry links exact versions into existing resource, record, map, graph and statistics views and does not claim to generate analytical answers.
