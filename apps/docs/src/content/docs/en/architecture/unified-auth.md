...
description: Current sign-in, project access, agent consent, and the shared identity rules for every WISER system.
...
| Task                      | Current route                                                              | What determines access                                                                                              |
| ------------------------- | -------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------- |
| Learn about WISER         | Portal is public                                                           | No account is needed to read system descriptions                                                                    |
| Enter a workspace         | Sign in with an existing organization account                              | A verified session and current project membership are required                                                      |
| Read or manage data       | Select a permitted project in Data Foundation                              | Membership, resource permission, source terms, and purpose are checked for the action                               |
| Join an exercise          | Open an authorized Agent EXCON scenario or run                             | The account needs the applicable exercise role and scope                                                            |
| Connect an external agent | Start OAuth from a compatible MCP client and review consent in the browser | The person chooses a project, purpose, access level, and term; web access does not automatically grant agent access |

Public self-registration is closed on the checked public service; account creation and project membership are handled by an administrator. [The public data guide](/en/development/wiser-data-guide/) describes the current entrypoints and the outstanding personal-client acceptance check. This page explains how the shared identity authority enforces those user journeys.
...
The production composition now loads the immutable resource authority after verified human/delegated resolution. Apply the control-plane resource migration and Data migration 0030 before starting this runtime; a missing or unavailable authority fails closed. Migrations do not enable any project. Each managed project still needs explicit activation, current source policies, and verification in its target environment. Explicit legacy mode preserves existing gates; managed mode requires a matching live grant. Existing session, project, scope, security-level, resource and provider checks remain mandatory in both modes. Do not accept evaluator authority inputs from browser requests or treat its expiry as permission to cache through a revocation.
...
### Fixed resource scope at MCP consent

For an explicitly managed project, successful user consent also binds the new Agent to the owner's currently active, unrevoked `agent-data` grants. It copies only their immutable package/preset versions, limits each expiry to the earlier human grant or Delegation expiry, and excludes Web-only, scheduled, expired and revoked grants. More than 1,000 active grants fails the entire consent. Project/settings locks use the same order as resource administration. Grant copies, provenance events, Agent/memberships, connection, Audit and Outbox commit together; an audit failure rolls everything back.

The consenting human approves this delegation. Each event retains the original grant and independent approver, plus the new connection/Delegation/Agent identities; it does not attribute a new approval to the original administrator. Every request still intersects this fixed scope with the human's live grants and current source limits. Newly approved resources require fresh consent, which revokes the previous Delegation. Removing one grant leaves any independently valid overlapping authorization effective within the original fixed scope and expiry. A managed connection with no approved MCP resources has no resource content access. Legacy projects keep their existing gates.
