...
vi.mock('./data-foundation-dal.server', () => ({
  getDataFoundationDal: () => Promise.resolve({ reconcile }),
...
import { POST } from '../app/api/data-foundation/reconciliation/[action]/route';
...
const call = (
  ...
  POST(
    new Request(
      'http://localhost/api/data-foundation/reconciliation/' + action,
      {
        method: 'POST',
        headers: {
          host: 'localhost',
...
it.each(['create', 'get', 'list', 'review'])(
  'forwards %s with its command identity and private response',
  async (action) => {
    reconcile.mockResolvedValue({ items: [] });
    const response = await call(action, '{"batchId":"example"}');
    expect(response.status).toBe(200);
    expect(response.headers.get('cache-control')).toBe('private, no-store');
    expect(reconcile).toHaveBeenCalledWith(action, { batchId: 'example' }, key);
  },
);
it('permits keyless reads and preserves access-denial status without disclosing upstream diagnostics', async () => {
  ...
  expect(reconcile).toHaveBeenCalledWith('get', {}, undefined);
  reconcile.mockRejectedValueOnce(new Error('private upstream diagnostic'));
  const unavailable = await call('review', '{}');
