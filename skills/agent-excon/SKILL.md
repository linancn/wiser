...
6. **Collaborate explicitly.** Send an `inform`, `request`, causal `response`, or ArtifactVersion `handoff` Message for bounded communication and publish an Artifact for reusable evidence or analysis. A response may reference only a request already issued through this RunAgent's own Receipt chain; a handoff pins exact ArtifactVersion identities and hashes. Choose only permitted recipient scopes; the platform freezes the recipient snapshot. Receipt acknowledgement is delivery state, never “read” or agreement. Update an Artifact from its declared base version and preserve both branches on conflict. Do not imply that shared team membership grants shared memory.
