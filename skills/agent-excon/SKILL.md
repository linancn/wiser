...
3. Observe through the API/MCP tool. Build the working evidence set only from Observation IDs actually returned for this Episode. Do not use future Injects, private outcomes, repository fixtures, or another participant's trace as evidence.
4. For the default Jing-Jin-Ji water-system exercise, read [yongding-allocation.md](references/yongding-allocation.md), construct a plan, and run `node scripts/validate-allocation-plan.mjs <plan.json>` before submitting.
