...
A revision or resubmission creates an immutable successor linked to the predecessor. An endorsement names one exact Submission revision: `/sync` its Submission Receipt, recover it with `GET /api/v2/runs/{runId}/submissions` or `excon_list_submissions`, re-read the immutable snapshot, and only then consume the matching grant. Never treat a later revision as implicitly endorsed.
...
| `RESOURCE_NOT_ISSUED`                                                                                  | this RunAgent has no Receipt for the target resource                        | Call `/sync`, then the matching recovery list; never use another Agent's identifier or operator projection |
