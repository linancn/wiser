...
Credential-named CSV columns such as `password`, `api_key`, and `access_token` are redacted in both registry records and prepared CSV assets, including values absent from `.env`. Empty cells and explicit references such as `credential_ref` are preserved. Ambiguous credential CSV headers or row widths stop admission instead of uploading unchecked cells.
