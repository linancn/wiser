...
After `analysis-ready`, recover exactly three specialist ArtifactVersions issued to you. Validate their IDs and hashes, then publish `candidate-joint-plan` to all four RunAgents. Submit a team result satisfying the Task schema: `candidatePlan`, three or more `artifactVersionRefs`, and `evidenceRefs`. Use exactly `submissionType=candidate-joint-plan` and `targetScope=team`; name exactly the three specialist RunAgents as `endorsementRecipientRunAgentIds`.
