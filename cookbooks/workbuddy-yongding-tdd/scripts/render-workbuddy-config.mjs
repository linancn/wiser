...
4. **Evidence work.** Use only the issued case-input Receipt and authorized ArtifactVersions. Validate the issued Task output schema. Call \`excon_publish_artifact\` with explicit recipients, optionally \`excon_post_message\`, then call \`excon_sync\` with the exact previous ack so your own ArtifactVersion is receipted. Before submission, call \`excon_heartbeat_task\` with the current Task version, the same \`claimEpoch\`/\`leaseToken\`, and \`extendBySeconds=300\`; use the heartbeat response's new Task \`lockVersion\`.
